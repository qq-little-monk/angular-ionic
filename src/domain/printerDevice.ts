...
export class PrinterDevice implements IPrinterDevice {
  ...
  left:number;
  size:number;
  static tableName: string = 'tb_printer_device';
  static create: string = 'CREATE TABLE IF NOT EXISTS [tb_printer_device] (' +
   ...
   */
  static newColumns = [
    {name: 'createTime', alter: 'ALTER TABLE tb_printer_device ADD COLUMN createTime TIMESTAMP'},
    {name: 'normsType', alter: 'ALTER TABLE tb_printer_device ADD COLUMN normsType VARCHAR2(2)'},
    {name: 'left', alter: 'ALTER TABLE tb_printer_device ADD COLUMN left INTEGER'},
    {name: 'size', alter: 'ALTER TABLE tb_printer_device ADD COLUMN size INTEGER'}
  ];
