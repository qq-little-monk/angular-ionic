...
export class MobileMessageService extends MessageServiceBase {
    ...
    pushMessageDeal() {
        let me = this;
        let bizType = me.data.bizType;
        let pushContent = me.data.data.content;
    ...
    orderRefresh(salesTableList, salesTable) {
        // debugger
        console.log('(salesTableList, salesTable', salesTableList, salesTable);

        if (salesTableList && salesTableList.length > 0) {
            for (let element of salesTableList) {
                if (this.appShopping.salesTable && this.appShopping.salesTable.id && (salesTable.id == element.id || salesTable.salesId == element.salesId)) {
                    if (salesTable.status != element.status) {
                        salesTable = element;
                        //订单状态跟新
                        this.events.publish('order:refresh');
                        console.log('order:refresh');
                        return;
                    } else if (salesTable != element) {
                        salesTable = element;
                        //订单数据更新跟新
                        this.events.publish('order:refresh-data');
                        console.log('order:refresh-data');
                        return;
                    }
                } else {
                    console.log('.....');
