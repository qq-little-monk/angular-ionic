...
import { AcceptOrder } from "../domain/acceptOrder";
import { Email } from "../domain/email";
import { Receive } from "../domain/receive";
import { SysMsg } from '../domain/sysMsg';
import { User } from '../domain/user';
import { SimpleStocking } from '../domain/simpleStocking';
import { StockingTop } from '../domain/stockingTop';
import { StockingPlan } from '../domain/stockingPlan';
...
const DB_NAME: string = 'cashier_app.db';
...
export class DbProvider {
  ...
  private createTables() {
    ...
    this.createTable(CommodityType);
    this.createTable(PrinterDevice);
    this.createTable(PrintSetting);
    this.createTable(AcceptOrder);
    this.createTable(Email);
    this.createTable(Receive);
    this.createTable(SimpleStocking);
    this.createTable(StockingTop);
    this.createTable(StockingPlan);
  ...
  set(tableName: string, entity: any, defaultDef: boolean = true): Promise<any> {
    ...
    return this.query(`insert or replace into ${tableName} (${keys.join(',')}) values (${params.join(",")})`, values)
      .then(data => {
