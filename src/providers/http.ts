...
export class HttpProvider {
  ...
  constructor(public httpClient: HttpClient, @Inject(APP_CONFIG) private config: AppConfig, public loadingCtrl: LoadingController, private datePipe: DatePipe,
    public nativeProvider: NativeProvider, public network: Network, public platform: Platform, public app: App, public appCache: AppCache, public appConstants: AppConstants) {
  ...
  static isMobile(mobile) {
    //定义手机号码正规表达式
    let reg = /^(130|131|132|133|134|135|136|137|138|139|140|141|142|143|144|145|146|147|148|149|150|151|152|153|155|156|157|158|159|166|176|180|181|182|183|184|185|186|187|188|189|198|199)\d{8}$/;
  ...
  public execRequest(observer, data: {
    ...
    me.loadingPresent(data.showLoading, data.content).subscribe(loading => {
      me.httpClient.request(data.method, data.url, { body: data.serialize, headers: headers }).timeout(me.appConstants.REQ_TIMEOUT).subscribe(res => {
  ...
  loadingPresent(isShowing: boolean, content: string, noSpinner: boolean = false, exitFlag: boolean = false): Observable<any> {
    return Observable.create(observer => {
      if (isShowing) {
        ...
        let promise = loading.present().then(() => {
        });
  ...
  loadingDismiss(loading) {
    if (loading && loading.dismiss) {
      loading.dismiss().catch((err) => { console.log("loadingDismiss: " + err); });
    }
  }
