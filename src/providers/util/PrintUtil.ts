...
export class PrintUtilProvider {
  ...
  public setkPCommodity(item: IOrderDetail): any[] {
    ...
    //菜品
    Observable.from(name).subscribe(char => {
      //中文占2个位置 ascii占1个位置
  ...
  public addRemark(remark: string, len: number = 18): any[] {
    ...
    let firstLine: boolean = true; //是否第一行
    this.push(data, "  ");
    Observable.from(remark).subscribe(char => {
      //中文占2个位置 ascii占1个位置
      char.charCodeAt(0) > 126 ? location += 2 : location += 1;
      //字符位置
      charCount += 1;
  ...
  stockingData(topAndLeg, stockingData, device: IPrinterDevice) {
    ...
    data = this.concatCmd(data, this.addLineSeparator());
    //详单
    Observable.from(stockingData).subscribe(item => {
      if (size && size != 1) {
        //80mmm
   ...
   */
  //客户小票 58mm 80mm
  salesData(topAndLeg, list, device: IPrinterDevice) {
    let size = device.size
    let num = 32
    if (size != 1) {
      num = 48
    ...
    //换行
    data = this.concatCmd(data, this.addLineSeparator());
    this.push(data, `服务员: ${(topAndLeg.createdName)}`);
    //换行
    data = this.concatCmd(data, this.addLineSeparator());
    this.push(data, `来源: 爱宝点菜宝`);
    //换行
  ...
  public setSales(item): any[] {
    ...
    if (!item.parentId) {//主商品
      ...
      let salesQty = item.salesQty;
      if (item.unitName) {
        salesQty = item.salesQty + item.unitName;
      }
      //加粗
      data = this.concatCmd(data, this.setFontBoldCmd(0));
      let n = this.util.stringPrintLength(salesQty + '');
      data = this.concatCmd(data, this.addSpace(6 - n));
      ...
      //加粗
      data = this.concatCmd(data, this.setFontBoldCmd(0));
      data = this.concatCmd(data, this.addSpace(24 - location - (item.salesQty + '').length));
      this.push(data, (item.salesQty + ''));
      //换行
      data = this.concatCmd(data, this.addLineSeparator());
      //取消加粗
      data = this.concatCmd(data, this.setFontBoldCmd(1));
  ...
  public setSales2(item): any[] {
    ...
    if (!item.parentId) {//主商品}
      ...
      let n = this.util.stringPrintLength(salesQty + '');
      data = this.concatCmd(data, this.addSpace(8 - n));
      this.push(data, (salesQty));
      ...
      //加粗
      data = this.concatCmd(data, this.setFontBoldCmd(0));
      data = this.concatCmd(data, this.addSpace(30 - location - (item.salesQty + '').length));
      this.push(data, (item.salesQty + ''));
      //换行
      data = this.concatCmd(data, this.addLineSeparator());
      //取消加粗
