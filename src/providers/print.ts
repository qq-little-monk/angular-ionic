...
export class PrintProvider {
  ...
  currAddress: string = "";
  constructor(public datePipe: DatePipe, @Inject(APP_CONFIG) private config: AppConfig, public appCache: AppCache, public bluetoothSerial: BluetoothSerial,
    ...
    return new Promise((resolve, reject) => {
      try {
        this.bluetoothSerial.disconnect().then(success => {
          ...
          console.log(success)
          //连接蓝牙打印机
          this.bluetoothSerial.connectInsecure(device.address).subscribe(connectSuccess => {
            console.log(connectSuccess)
            this.currAddress = device.address;
            console.log(22)
            //打印
            this.bluetoothSerial.write(buf).then(success => {
              for (let i = 1; i < printNum; i++) {
                //打印
                this.bluetoothSerial.write(buf);
              }
              resolve({});
            }, failure => {
  ...
  private doBleWrite(device, buf, printNum) {
    return new Promise((resolve, reject) => {
      try {
        //连接蓝牙打印机
        this.ble.connect(device.address).subscribe(connectSuccess => {
          console.log(connectSuccess);
          ...
          console.log(22)
          //打印
          this.ble.write(device.address, device.servicesId, device.characteristicId, buf).then(success => {
            for (let i = 1; i < printNum; i++) {
              //打印
              this.ble.write(device.address, device.servicesId, device.characteristicId, buf);
            }
            resolve({});
          }, failure => {
            this.nativeProvider.showShortCenter('打印机连接失败');
  ...
  private bleConnectAndWrite(bufList, device: IPrinterDevice, printNum: number = 1): Promise<any> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(device);
        this.ble.isConnected(device.address).then(() => {
        ...
        this.ble.connect(device.address).subscribe(data => {
          console.log('连接返回数据');
          console.log(data);
          ...
          for (let item of bufList) {
            this.ble.write(device.address, device.servicesId, device.characteristicId, item).then(success => {
              console.log(success);
              console.log(' 进入打印方法+=============================1');
              for (let i = 1; i < printNum; i++) {
                //打印
                console.log('执行打印方法+=============================1');
                this.ble.write(device.address, device.servicesId, device.characteristicId, item);
              }
              resolve({});
            }, failure => {
              reject({ errType: '2' });
              ...
              // this.ble.writeWithoutResponse(device.address, device.servicesId, device.characteristicId, buf);
            })
          }
  ...
  stockingReceiptPrint(topAndLeg, list, device: IPrinterDevice, dataType:string='') { 
    ...
    this.stockingPrint(topAndLeg, list, device, 1, dataType).then(res => {
      ...
      setTimeout(res => {
        if (device.linkType == String(linkType.bluetooth) && this.bluetoothSerial.isConnected()) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (this.platform.versions().ios) {
          ...
          if (device.linkType == String(linkType.bluetooth)) {
            this.bleConnectAndWrite(bufList, device, printNum).then(res => {
              resolve({});
            }, err => {
          ...
          if (dataType == 'sales') {//销售数据
            data = this.printUtil.salesData(topAndLeg, list, device);//stockingData中判断58还是80
          } else {//盘点数据
          ...
          if (device.linkType == String(linkType.bluetooth)) {
            if (this.platform.versions().ios) {
              this.bleConnectAndWrite(buf, device, printNum).then(res => {
                resolve({});
              }, err => {
                reject({ errType: err.errType });
              })
  ...
  testReceiptPrint(device: IPrinterDevice) {
    ...
    this.stockingPrint(topAndLeg, list, device).then(res => {
      ...
      setTimeout(res => {
        ...
        if (this.platform.versions().ios) {
          if (device.linkType == String(linkType.bluetooth) && this.ble.isConnected(device.address)) {
